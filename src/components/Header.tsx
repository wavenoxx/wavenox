...
export function Header() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-40 bg-transparent flex justify-between items-center px-4 lg:px-10 py-6">
        <a
          href="#"
          className="text-white text-xl lg:text-2xl font-bold uppercase tracking-[0.4em] select-none"
        >
          WAVENOX
        </a>
        ...
        <nav className="hidden xl:flex items-center space-x-5 2xl:space-x-7">
          {NAV_ITEMS.map((item) => (
            <a
              key={item}
              href="#"
              className="relative text-white text-[11px] font-bold uppercase tracking-widest group"
            >
              {item}
